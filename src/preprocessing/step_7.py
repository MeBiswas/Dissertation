...
def visualize_preprocessing(
    ...
    panels = [
        (axes[0,0], cv2.cvtColor(original_color, cv2.COLOR_BGR2RGB), '(a) Original pseudo-colour TBI', False),
        (axes[0,1], cv2.cvtColor(without_scale,  cv2.COLOR_BGR2RGB), '(b) Overlay stripped + no colour bar', False),
        (axes[0,2], grayscale, '(c) Blue channel — hot = bright', True),
        (axes[1,0], bg_removed, '(d) Background removed', True),
        (axes[1,1], pb, '(e) p_b — after reconstruction\n(input to SCH-CS)', True),
    ]
    ...
    ax_h.axvline(nz.mean(), color='red', ls='--', label=f'mean = {nz.mean():.1f}')
