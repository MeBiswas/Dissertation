...
# Adjusting threshold but not working
...
#     # Modified strategy based on observation that m(p) can be too high.
#     # Prioritize t* if it's a positive and meaningful value.
#     # Ensuring t* is a standard float type for comparison to avoid potential subtle issues.
#     if float(t_star) > 0.0: 
#         th = t_star
#         reason = f"Using t* ({t_star:.4f}) as final threshold (t* > 0)"
#     else:
#         # If t* is not positive, it might not be a meaningful threshold for 'hot' regions.
#         # In this case, fall back to m(p) as a more general intensity measure.
#         th = m_p
#         reason = f"t* ({t_star:.4f}) is <= 0, falling back to m(p) ({m_p:.4f}) as final threshold"

#     print(f"\n  Equation 3 decision (Modified Strategy):")
#     print(f"  {reason}")
#     print(f"\n  th = {th:.4f}")
#     print(f"\n  Pixels with intensity > {th:.1f} → classified as SR")
#     print(f"  Pixels with intensity <= {th:.1f} → classified as TBR/background")
...
#     print(f"  {'m(p)':<12}  {'mean of non-zero pixels in pb':<35}  {m_p:.4f}")
#     print(f"  {'t*':<12}  {'from Step 2':<35}  {t_star:.4f}")
#     print(f"  {'th':<12}  {'Modified from max(t*, m(p))':<35}  {th:.4f}")
