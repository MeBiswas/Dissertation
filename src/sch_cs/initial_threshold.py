...
def compute_initial_threshold(step1: dict, plot: bool = True) -> dict:
    ...
    if j == 0:
        ...
        return {"A": A_levels, "j": 0, "m_A": None, "alpha_A": None, "t_star": None}
    ...
    if plot:
        ...
        fig.suptitle("SCH-CS Step 2 — Initial Threshold t*", fontsize=13, fontweight='bold')
 
        # Left: full histogram with rho_effective and A highlighted
        ax = axes[0]
        full_hist = step1["histogram"].copy()
        full_hist[0] = 0
        ax.bar(range(256), full_hist, color='steelblue', alpha=0.5, width=1.0, label='h(ni)')
        ax.axhline(y=rho_effective, color='red', linewidth=2, linestyle='-.', label=f'rho_effective = {rho_effective:.1f}')
        highlight = np.zeros(256)
        
        for gl, freq in zip(A_levels, A_freqs):
            highlight[gl] = freq
        
        ax.bar(range(256), highlight, color='orange', alpha=0.8, width=1.0, label=f'Array A ({j} levels)')
        ...
        ax2 = axes[1]
        ax2.bar(A_levels, A_freqs, color='orange', alpha=0.8, width=1.0, label='A gray levels')
        ax2.axvline(x=m_A, color='green', linewidth=2, linestyle='--', label=f'm(A) = {m_A:.2f}')
        ax2.axvline(x=t_star, color='purple', linewidth=2, linestyle='-.', label=f't* = {t_star:.2f}')
        ax2.axvline(
            x=m_A + alpha_A,
            color='green',
            linewidth=1,
        ...
        ax2.axvspan(t_star, m_A + alpha_A, alpha=0.1, color='green', label='±1 std range')
